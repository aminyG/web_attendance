...
@section('content')
    <div class="pcoded-main-container">
        <div class="pcoded-content">
            <h2 class="text-2xl font-semibold mb-4">Edit Lokasi</h2>
            ...
            <form method="POST" action="{{ route('locations.update', $location) }}">
                ...
                <input type="hidden" name="latitude" id="latitude" value="{{ $location->latitude }}" required>
                <input type="hidden" name="longitude" id="longitude" value="{{ $location->longitude }}" required>

                <div class="mt-3">
                    <label>Jarak radius :</label>
                    <select name="radius" id="radius" class="form-control" required>
                        @foreach([10, 20, 30, 50, 100] as $r)
                            <option value="{{ $r }}" {{ $location->radius == $r ? 'selected' : '' }}>{{ $r }} Meter</option>
                        @endforeach
                    </select>
                </div>

...
public function index()
    {
        ...
        // Ambil lokasi yang aktif
        $locations = Location::where('is_active', true)->get(); // Pastikan hanya mengambil lokasi yang aktif

        // Kirimkan data kategori dan lokasi ke view
        return view('schedule.index', compact('categories', 'locations'));
    }
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'location_id' => 'required|exists:locations,id',  // Validasi location_id
        ...
        // Simpan data jadwal di database lokal (tanpa presence_id terlebih dahulu)
        ...
        try {
            $apiResponse = \Illuminate\Support\Facades\Http::withHeaders([
                ...
                'schedule_type' => 'daily',  // Sesuaikan jika ada tipe lainnya
                'schedule_day' => null,      // Ganti jika diperlukan, misalnya untuk `weekly` atau `monthly`
            ]);

            if ($apiResponse->successful()) {
                $presenceId = $apiResponse->json('data.id'); // Mendapatkan ID dari response API

                // Update database lokal dengan presence_id
                $schedule->update([
    ...
    public function update(Request $request, $id)
    {
        ...
        // Update jadwal di database lokal dengan data baru
